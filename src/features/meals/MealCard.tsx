        ...
        <div className={`card-glass h-full flex flex-col relative overflow-hidden transition-all duration-300 ${isSwapping ? 'opacity-50 scale-95 blur-sm' : ''}`}>
            ...
            <div className="flex justify-between items-start mb-4 relative z-10">
                <div className="flex items-center gap-2">
                    ...
                    <div>
                        <span className="text-xs text-emerald-400 font-medium">{mealNameHebrew}</span>
                        <h4 className="font-bold text-lg text-white leading-tight">{mealData.name}</h4>
                    </div>
                </div>
                ...
                <ul className="text-xs text-muted space-y-2">
                    {mealData.ingredients.slice(0, 4).map((item, i) => (
                        <li
                            ...
                            className="flex items-center gap-2 p-1.5 rounded bg-surface/30 hover:bg-surface-hover cursor-pointer transition-colors border border-transparent hover:border-emerald-500/30 group"
                        >
                            <span className="text-emerald-500/50 group-hover:text-emerald-400 text-[10px]">✏️</span>
                            {item}
                        </li>
                    ))}
                    {mealData.ingredients.length > 4 && (
                        <li className="pl-6 text-muted/70 italic mt-1">ועוד {mealData.ingredients.length - 4} מצרכים...</li>
                    )}
                </ul>
            </div>

            <div className="flex justify-between items-center mt-auto pt-4 border-t border-surface-hover">
